# CLAUDE.md — Guia do repositório G3 Educação | Saúde
...
React+Vite (aluno, backoffice) · Next.js (site) · Tailwind + design system G3 ·
...
packages/
  shared/         # tipos, schemas Zod, utils (@g3/shared)
  api-client/     # cliente tipado, gerado do OpenAPI (@g3/api-client)
  design-tokens/  # tokens da marca G3 (@g3/design-tokens)
  ui/             # componentes base React do DS (@g3/ui)
infra/           # docker-compose (postgres, redis), Caddyfile (ingress)
docs/
  adr/           # decisões arquiteturais (0001–0010)
  architecture.md# topologia e grafo de dependências
  prototype/     # mockup HTML aprovado (referência visual)
  brand/         # logotipos G3 (lockup, negativa, símbolo)
```
...
- **Marca**: Manual de Identidade Visual G3 v1.0 — Azul G3 `#001133` domina (65%),
  Ouro G3 `#C9A04A` é acento (10%), branco/marfim garante respiro (25%). Títulos em
  Cormorant Garamond, texto e rótulos em Josefin Sans (rótulos em caixa-alta com
  0,2–0,3 em de espaçamento). Tokens em `packages/design-tokens` — não hardcodar cor.

## Estado atual

Plataforma portada da base Vethis: API, site, área do aluno e backoffice já de pé,
com paleta, tipografia, logotipos e catálogo trocados para a G3. O catálogo do seed
(`apps/api/src/db/seed.ts`) é conteúdo de demonstração — cursos, docentes e números
são fictícios até a coordenação publicar os reais pelo backoffice.
