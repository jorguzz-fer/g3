...
const CARD_CLASS =
  'rounded-3xl border border-gold-400/30 bg-gradient-to-br from-navy-800 to-navy-900 p-7 text-[#EAECF0] shadow-[0_20px_50px_rgba(2,20,12,.35)]';

/**
 * Card de oferta da página do curso: preço cheio, parcela em destaque e CTA.
 * Cores da marca G3 (verde escuro + dourado). Reutilizado no hero, no card
...
export function OfferCard({
  ...
  if (comingSoon) {
    return (
      <div className={`${CARD_CLASS} ${className}`}>
        ...
        <h3 className="mt-4 font-serif text-2xl font-semibold text-[#EAECF0]">
          Matrículas em breve
        </h3>
        <p className="mt-3 text-[15px] leading-relaxed text-[#C6CAD3]">
  ...
  return (
    <div className={`${CARD_CLASS} ${className}`}>
      ...
      <h3 className="mt-4 font-serif text-2xl font-semibold text-[#EAECF0]">Investimentos</h3>

      <p className="mt-3 text-[15px] text-[#C6CAD3]">
        Por: <span className="font-semibold text-white">{formatBRL(course.priceCents)}</span>
      </p>

      <p className="mt-5 text-[15px] text-[#C6CAD3]">
      ...
      <p className="mt-5 text-xs leading-relaxed text-[#9DA3B0]">
        No Pix à vista: <span className="font-medium text-[#EAECF0]">{formatBRL(pixCents)}</span>,
        5% de desconto
      </p>
