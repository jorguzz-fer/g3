...
import { Badge, Button, Field } from '@g3/ui';
...
function AuthPanel({ onAuthed }: { onAuthed: (u: AuthUser) => void }) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-white p-6 shadow-sm sm:p-8">
      ...
      <h1 className="mt-1 font-serif text-2xl font-semibold text-navy-800">
        ...
        {(['register', 'login'] as const).map((m) => (
          <button
            ...
            className={`rounded-lg px-4 py-1.5 text-sm font-semibold transition-colors ${
              mode === m ? 'bg-navy-700 text-white' : 'text-muted hover:text-navy-700'
            }`}
          >
...
function PaymentPanel({
  ...
  return (
    <section className="rounded-2xl border border-border bg-white p-6 shadow-sm sm:p-8">
      ...
      <h1 className="mt-1 font-serif text-2xl font-semibold text-navy-800">Pagamento</h1>
          ...
          <button
            ...
            className={`rounded-xl border p-3 text-left transition-colors ${
              method === m.key
                ...
                : 'border-border hover:border-navy-700/40'
            }`}
          >
    ...
    <section className="rounded-2xl border border-border bg-white p-6 shadow-sm sm:p-8">
      ...
      {order.method === 'boleto' && order.boletoUrl ? (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-5 inline-flex items-center gap-2 font-semibold text-navy-700 hover:underline"
        >
          Abrir boleto →
        </a>
      ) : null}
...
function SuccessPanel({ course }: { course: CourseDetail }) {
  return (
    <section className="rounded-2xl border border-border bg-white p-6 text-center shadow-sm sm:p-10">
      <div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-navy-700 text-white">
        <svg
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
      ...
      <h1 className="mt-5 font-serif text-2xl font-semibold text-navy-800">
        Matrícula liberada! 🎉
      </h1>
      ...
      <p className="mt-3 text-sm">
        <Link href="/cursos" className="text-navy-700 hover:underline">
          Ver mais cursos
        </Link>
      </p>
    </section>
...
function OrderSummary({
  ...
  return (
    <aside className="h-fit rounded-2xl border border-border bg-white p-6 shadow-sm lg:sticky lg:top-6">
      ...
      <h2 className="mt-2 font-serif text-lg font-semibold text-navy-800">{course.title}</h2>
      {course.instructor ? (
      ...
      <div className="mt-4 border-t border-border pt-4">
        ...
        {pixDiscount > 0 ? (
          <div className="mt-2 flex items-center justify-between text-navy-700">
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <span className="font-serif text-xl font-semibold text-navy-800">{formatBRL(total)}</span>
        </div>
        {parcels > 1 ? (
