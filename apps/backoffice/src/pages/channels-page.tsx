...
export function ChannelsPage() {
  ...
  return (
    <div>
      <h1 className="mb-4 font-serif text-3xl font-semibold text-navy-800">Canais de aquisição</h1>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        {channel.rules.length === 0 ? (
          ...
          channel.rules.map((r) => (
            <span
              key={r.id}
              className="inline-flex items-center gap-1 rounded-full bg-navy-50 px-2.5 py-1 text-xs text-navy-800"
            >
              {r.utmSource}
              ...
              <button
                ...
                className="text-navy-700 hover:text-error"
              >
                ×
              </button>
            </span>
...
function ChannelModal({
  ...
  const [color, setColor] = useState(editing?.color ?? '#345089');
...
function UtmBuilder() {
  ...
  return (
    <div className="max-w-xl">
      ...
      <div className="mt-5 rounded-lg border border-border bg-navy-50/50 p-4">
