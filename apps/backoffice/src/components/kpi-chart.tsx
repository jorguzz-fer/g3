...
// Marca G3: série única em verde (sem legenda — o título nomeia a série).
const LINE = '#1C3466'; // navy-700
const AREA = '#1C3466';
const GRID = '#E4E2DA'; // border
const INK = '#141A26';
const MUTED = '#4A5570';
  ...
  return (
    <div className="rounded-lg border border-border bg-white p-5">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
        <h3 className="font-serif text-lg font-semibold text-navy-800">{active.title}</h3>
          ...
          {METRICS.map((m) => (
            <button
              ...
              className={`rounded-md px-3 py-1 text-sm font-medium transition-colors ${
                metric === m.key ? 'bg-navy-700 text-white' : 'text-muted hover:text-ink'
              }`}
            >
              {m.label}
            </button>
