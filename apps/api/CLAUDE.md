...
```bash
pnpm --filter @g3/api dev          # tsx watch (requer .env + Postgres no ar)
pnpm --filter @g3/api build        # tsc → dist
pnpm --filter @g3/api test         # vitest
pnpm --filter @g3/api db:generate  # gera migration a partir do schema
pnpm --filter @g3/api db:migrate   # aplica migrations
pnpm --filter @g3/api db:seed      # seed de dev (idempotente)
```
...
- **M1d** geração do `@g3/api-client` a partir do contrato; domínios orders/enrollment.
...
(sem drift). `pnpm --filter @g3/api openapi:generate` escreve `apps/api/openapi.json`
