...
import { Button, ProgressBar } from '@g3/ui';
...
function LessonStatus({ completed, active }: { completed: boolean; active: boolean }) {
  if (completed) {
    return (
      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-navy-700 text-white">
        <CheckIcon />
      </span>
    );
  }
  return (
    <span
      className={`grid h-7 w-7 shrink-0 place-items-center rounded-full border ${
        active ? 'border-navy-700 text-navy-700' : 'border-border text-muted'
      }`}
    >
      <PlayIcon />
    </span>
...
export function CoursePlayerPage() {
  ...
  return (
    <div>
      <Link
        to="/meus-cursos"
        className="mb-3 inline-flex items-center gap-1 text-sm font-semibold text-navy-700 hover:underline"
      >
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="min-w-0">
          <h1 className="mb-1 font-serif text-2xl font-semibold text-navy-800">{course.title}</h1>
          {course.subtitle ? (
          ...
          <div className="mb-5 aspect-video overflow-hidden rounded-lg bg-navy-900 shadow-[0_12px_34px_-18px_rgba(10,31,25,.35)]">
            {current?.vimeoVideoId ? (
              ...
              <div className="grid h-full place-items-center text-sm text-[#9DA3B0]">
                          ...
                          <li key={l.id}>
                            <button
                              ...
                              className={`flex w-full items-center gap-3 px-4 py-3 text-left transition-colors ${
                                active ? 'bg-navy-50' : 'hover:bg-navy-50/50'
                              }`}
                            >
                              <span
        ...
        <aside className="flex flex-col gap-5">
          {course.instructor ? (
            <div className="flex items-center gap-3 rounded-lg border border-border bg-white p-5">
              {course.instructor.avatarUrl ? (
                ...
                <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-navy-700 font-serif text-lg font-semibold text-gold-400">
          ...
          <div className="rounded-lg border border-border bg-white p-5">
            <h3 className="mb-3 font-serif text-base font-semibold text-navy-800">
              Progresso do curso
            </h3>
          ...
          {otherCourses.length > 0 ? (
            <div className="rounded-lg border border-border bg-white p-5">
              <h3 className="mb-4 font-serif text-base font-semibold text-navy-800">Meus cursos</h3>
